...
public:
    ...
    /**
        ...
        @return <void>
    */
    void Insert( const TK& newKey, const TD& newData )//done
    {
        if (m_ptrRoot == nullptr)
        {
...
//! Searches the tree for the key provided and returns true if found, false if not.
    ...
    */
    bool Contains( const TK& key ) //done
    {
        return RecursiveContains(key, m_ptrRoot);
    }

    bool RecursiveContains(const TK& key, Node<TK,TD>* ptrCurrent) //done
    {
        ...
        return (RecursiveContains (key, ptrCurrent->ptrLeft) || RecursiveContains (key, ptrCurrent->ptrRight));
...
//! Returns the Node* that contains the key, or nullptr if data is not in the tree.
    ...
    */
    Node<TK,TD>* FindNode( const TK& key ) //done
    {
    ...
    Node<TK,TD>* RecursiveFindNode (const TK& key, Node<TK,TD>* ptrCurrent)
    {
        ...
        Node<TK,TD>* FindNodeRight = RecursiveFindNode(key, ptrCurrent->ptrRight);
        Node<TK,TD>* FindNodeLeft = RecursiveFindNode(key, ptrCurrent->ptrLeft);

        if (FindNodeRight != nullptr)
        {
            return FindNodeRight;
        }
        if (FindNodeLeft != nullptr)
        {
            return FindNodeLeft;
        }
        else
...
//! Returns the PARENT Node* of the Node* that contains the key, or nullptr if data is not in the tree.
    ...
    */
    ...
    {
        return RecursiveFindParentNode(key, m_ptrRoot);
    }

    Node<TK,TD>* RecursiveFindParentNode (const TK& key, Node<TK,TD>* ptrCurrent) //done
    {
        if (ptrCurrent == nullptr)
        {
            return nullptr;
        }
        if (ptrCurrent->ptrLeft != nullptr && ptrCurrent->ptrLeft->key == key)
        {
            return ptrCurrent;
        }
        if (ptrCurrent->ptrRight != nullptr && ptrCurrent->ptrRight->key == key)
        {
            return ptrCurrent;
        }

        Node<TK,TD>* FindNodeRight = RecursiveFindParentNode(key, ptrCurrent->ptrRight);
        Node<TK,TD>* FindNodeLeft = RecursiveFindParentNode(key, ptrCurrent->ptrLeft);
...
//! Recurses through the tree and finds the proper location for the new data.
    /**
        ...
        @return <void>
    */
    void RecursiveInsert( const TK& newKey, const TD& newData, Node<TK, TD>* ptrCurrent ) //done
    {
...
//! Recurses through the tree in IN-ORDER order, writing to the stream.
    ...
    */
    void GetInOrder( Node<TK, TD>* ptrCurrent, stringstream& stream ) //done
    {
        if (ptrCurrent != nullptr)
        {
...
//! Recurses through the tree in PRE-ORDER order, writing to the stream.
    /**
        ...
        at the end of the generated string.
    */
    void GetPreOrder( Node<TK, TD>* ptrCurrent, stringstream& stream ) //done
    {
        if ( ptrCurrent != nullptr )
        {
            stream << ptrCurrent->key << " ";
...
//! Recurses through the tree in POST-ORDER order, writing to the stream.
    /**
        ...
        at the end of the generated string.
    */
    void GetPostOrder( Node<TK, TD>* ptrCurrent, stringstream& stream ) //done
    {
        if (ptrCurrent != nullptr)
        {
...
//! Recurses through the tree, going to the right-child-nodes until the max key is found.
    ...
    */
    TK* GetMax( Node<TK, TD>* ptrCurrent ) //done
    {
        if (ptrCurrent->ptrRight != nullptr)
        {
    ...
    Call recur­sively.
    */
    int GetHeight( Node<TK, TD>* ptrCurrent ) //done
    {
        if ( ptrCurrent == nullptr )
        {
            return 0;
