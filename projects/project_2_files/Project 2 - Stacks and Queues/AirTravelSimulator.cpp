...
void AirTravelSimulator::Run()
{
    ...
    cout << "Max capacity: " << m_airport.GetMaxCapacity() << endl;
...
bool AirTravelSimulator::LineUp()
{
    ...
    Traveller* random_traveller;
    random_traveller = m_pplManager.GetTraveller(random_index);
    m_airport.LineUp(random_traveller);
    m_ptrTravellers.PushBack(random_traveller);;
    random_traveller->state = LINEUP;
    DisplayMessage(random_traveller, LINEUP);

    cout << "People waiting: " << m_airport.WaitingCount() << endl;
    cout << "Max capacity:   " << m_airport.GetMaxCapacity() << endl;
...
{
    Traveller* known_traveller;
    known_traveller = m_airport.NextInLine();
    m_airplane.Board(known_traveller);
    known_traveller->state = BOARDING;
    DisplayMessage(known_traveller, BOARDING);
...
{
    Traveller* unhappy_passenger;
    unhappy_passenger =m_airplane.Disembark();
    unhappy_passenger->state = FINISHED;
    DisplayMessage(unhappy_passenger, DISEMBARKING);
